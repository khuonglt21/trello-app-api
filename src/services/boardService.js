...
const getById = async (id, callback) => {
    try {
        // Get board by id
        const board = await boardModel.findById(id).populate({path: "teams", select: ["members", "name"]});
...
const changeVisibilityOfBoard = async (req, callback) => {
    try {
        req.body.listMember.map(async (member) => {
            const user = await userModel.findById(member.user)
            const IdBoardChangeVisibility = user.boards.filter(boardId => boardId.toString() === req.body.boardId);
            const FindBoardChangeVisibility = await boardModel.findById(IdBoardChangeVisibility)
            FindBoardChangeVisibility.visibility = req.body.valueVisibility;
            await FindBoardChangeVisibility.save();
        })
        return callback(false, {message: 'Success!'});
    } catch (error) {
        return callback({message: 'Something went wrong', details: error.message});
